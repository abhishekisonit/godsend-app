  ...
  private async waitForContainersToBeReady(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (
          psOutput.includes('postgres_db') &&
          psOutput.includes('Up') &&
          (psOutput.includes('healthy') || psOutput.includes('(healthy)'))
        ) {
          this.log(
            `Containers are ready! (attempt ${attempt}/${maxAttempts})`,
            'success'
          );
          return;
        }
        ...
        if (attempt === maxAttempts) {
          throw new Error(
            'Containers failed to start properly within expected time'
          );
        }

        if (this.options.verbose) {
          this.log(
            `Containers not ready yet... (attempt ${attempt}/${maxAttempts})`,
            'warning'
          );
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        await this.runCommand(
          'docker exec postgres_db pg_isready -U postgres -d postgres'
        );
        this.log(
          `Database is ready! (attempt ${attempt}/${maxAttempts})`,
          'success'
        );
        return;
      } catch (error) {
  ...
  private async pushSchema(): Promise<void> {
    ...
    try {
      ...
      if (this.options.verbose) {
        ...
        this.log(
          `With environment: DATABASE_URL=postgresql://postgres:prisma@postgres_db:5432/postgres?schema=public`,
          'info'
        );
      }
