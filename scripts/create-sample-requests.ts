...
// Sample data arrays
const categories: CreateRequestData['category'][] = [
  'Food',
  'Medicine',
  'Clothing',
  'Electronics',
...
function getItemsForCategory(
  category: CreateRequestData['category']
): string[] {
  switch (category) {
    case 'Food':
      return foodItems;
...
function generateSampleRequest(): CreateRequestData {
  ...
  return {
    ...
    alternativeSource:
      Math.random() > 0.7 ? `Alternative shop in ${sourceCity}` : undefined,
...
async function createTestUsers(): Promise<void> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(
        '💡 Run "npm run create-test-users" first to create test users'
      );
      throw new Error(
        'Test user not found. Please run "npm run create-test-users" first.'
      );
    }
  } catch (error) {
