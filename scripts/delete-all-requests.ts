...
async function deleteRequest(requestId: string, apiKey: string): Promise<any> {
  try {
    const response = await fetch(
      `http://localhost:3000/api/requests/${requestId}/delete`,
      {
        method: 'DELETE',
        headers: {
...
async function deleteAllRequests() {
  ...
  try {
    ...
    for (let i = 0; i < requests.length; i++) {
      ...
      try {
        console.log(
          `Deleting request ${i + 1}/${requests.length}: "${request.title}" (${request.status})`
        );
...
if (require.main === module) {
  ...
  console.log(
    '⚠️  DEVELOPMENT ONLY: This script deletes test data and should not be used in production!'
  );
