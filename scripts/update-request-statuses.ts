...
async function getAllRequests(apiKey: string): Promise<any[]> {
  try {
    ...
    while (true) {
      const response = await fetch(
        `http://localhost:3000/api/requests?limit=${limit}&offset=${offset}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
...
async function updateRequestStatuses() {
  ...
  try {
    ...
    if (requests.length === 0) {
      console.log(
        '❌ No requests found. Please run "npm run create-sample-requests" first.'
      );
      return;
    }
    ...
    const statuses: UpdateStatusData['status'][] = [
      'OPEN',
      'IN_PROGRESS',
      'COMPLETED',
      'CANCELLED',
      ...
      if (apiKey) {
        updateRequestStatus(request.id, status, apiKey)
          ...
          .catch(error => {
            ...
            console.error(
              `Failed to update request ${request.id}:`,
              errorMessage
            );
          });
  ...
  try {
    ...
    if (apiKey) {
      ...
      try {
        ...
        console.log(
          "⚠️ Could not invalidate session (this is normal if endpoint doesn't exist)"
        );
      }
...
if (require.main === module) {
  ...
  if (nodeEnv === 'production') {
    console.error(
      '❌ PRODUCTION BLOCKED: This script is for development only!'
    );
  ...
  console.log(
    '⚠️  DEVELOPMENT ONLY: This script updates test data and should not be used in production!'
  );
