  ...
  try {
    ...
    if (existingRequest.status !== 'OPEN') {
      return NextResponse.json(
        { error: 'Request is not available for fulfillment' },
        { status: 400 }
      );
    }
    ...
    if (existingRequest.requesterId === user.id) {
      return NextResponse.json(
        { error: 'Cannot fulfill your own request' },
        { status: 400 }
      );
    }
    ...
    if (existingRequest.fulfillerId === user.id) {
      return NextResponse.json(
        { error: 'You are already fulfilling this request' },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      error.message === 'Request is no longer available'
    ) {
      return NextResponse.json(
