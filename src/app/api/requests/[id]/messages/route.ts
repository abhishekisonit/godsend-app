...
export async function GET(
  ...
  try {
    ...
    if (
      ...
      existingRequest.fulfillerId !== user.id
    ) {
      return NextResponse.json(
        { error: 'Not authorized to view messages for this request' },
        { status: 403 }
      );
    }
...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Not authorized to send messages for this request' },
        { status: 403 }
      );
    }
    ...
    if (existingRequest.status === 'CANCELLED') {
      return NextResponse.json(
        { error: 'Cannot send messages for cancelled requests' },
        { status: 400 }
      );
    }
