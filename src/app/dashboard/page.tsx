...
import { Box, Stack, Heading, Text, Container, Alert } from '@chakra-ui/react';
  ...
  return (
    <Container maxW="lg" py={10}>
      <Stack gap={8}>
              ...
              <Alert.Root status={dbStatus.success ? 'success' : 'error'}>
                ...
                {dbStatus.data && (
                  <Text fontSize="sm" mt={1}>
                    Tables: {dbStatus.data.tableCount || 0} | Users:{' '}
                    {dbStatus.data.userCount || 0}
                  </Text>
                )}
              </Alert.Root>
            )}
        ...
        <Card title="Account Actions" subtitle="Manage your account">
          <Stack gap={3}>
            <Button variant="secondary" w="full">
              Edit Profile
            </Button>
            <Button variant="danger" onClick={logout} w="full">
              Sign Out
            </Button>
          </Stack>
        </Card>
