...
export async function authenticateRequest(request: NextRequest): Promise<AuthenticatedRequest | null> {
    try {
        ...
        console.log('🔐 [AUTH] Session user email:', session?.user?.email || 'none');
        ...
        if (session?.user?.email) {
            ...
            if (user && user.email) {
                console.log('🔐 [AUTH] ✅ JWT authentication successful for user:', user.email);
                ...
                console.log('🔐 [AUTH] ❌ JWT authentication failed - user not found in database');
            }
        } else {
            console.log('🔐 [AUTH] ⚠️ No NextAuth session found, checking for API key...');
        }
        ...
        if (apiKey) {
            ...
            try {
                ...
                const decodedToken = JSON.parse(Buffer.from(apiKey, 'base64').toString());
                console.log('🔐 [AUTH] API key decoded successfully for user:', decodedToken.email);

                // Check if token is expired
                if (decodedToken.exp && Date.now() > decodedToken.exp) {
                    console.log('🔐 [AUTH] ❌ API key authentication failed - token expired');
                    return null;
                }
                ...
                if (user && user.email) {
                    console.log('🔐 [AUTH] ✅ API key authentication successful for user:', user.email);
                    (request as AuthenticatedRequest).user = {
                        id: user.id,
                        email: user.email,
                        name: user.name || undefined,
                    ...
                    console.log('🔐 [AUTH] ❌ API key authentication failed - user not found in database');
                }
            } catch (error) {
        ...
        console.log('🔐 [AUTH] ❌ Authentication failed - no valid session or API key');
        return null;
    } catch (error) {
...
export function createAuthMiddleware() {
    return async function authMiddleware(request: NextRequest): Promise<NextResponse | null> {
        ...
        console.log('🔐 [AUTH] ✅ Authentication successful, continuing to handler');
